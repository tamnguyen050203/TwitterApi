...
`
  ...
  ffmpeg -y -i ${inputPath} -preset veryslow -g 48 -crf 17 -sc_threshold 0 -map 0:0 -map 0:1 -map 0:0 -map 0:1 -s:v:0 ${getWidth(
    720,
    resolution
  )}x720 -c:v:0 libx264 -b:v:0 ${bitrate720} -s:v:1 ${getWidth(
    1080,
    resolution
  )}x1080 -c:v:1 libx264 -b:v:1 ${bitrate1080} -c:a copy -var_stream_map "v:0,a:0 v:1,a:1" -master_pl_name master.m3u8 -f hls -hls_time 6 -hls_list_size 0 -hls_segment_filename "${outputSegmentPath}" ${outputPath}
`
  const commandWithMax1440 = `
  ffmpeg -y -i ${inputPath} -preset veryslow -g 48 -crf 17 -sc_threshold 0 -map 0:0 -map 0:1 -map 0:0 -map 0:1 -map 0:0 -map 0:1 -s:v:0 ${getWidth(
    720,
    resolution
  )}x720 -c:v:0 libx264 -b:v:0 ${bitrate720} -s:v:1 ${getWidth(
    1080,
    resolution
  )}x1080 -c:v:1 libx264 -b:v:1 ${bitrate1080} -s:v:2 ${getWidth(
    1440,
    resolution
  )}x1440 -c:v:2 libx264 -b:v:2 ${bitrate1440} -c:a copy -var_stream_map "v:0,a:0 v:1,a:1 v:2,a:2" -master_pl_name master.m3u8 -f hls -hls_time 6 -hls_list_size 0 -hls_segment_filename "${outputSegmentPath}" ${outputPath}
`

  const commandWithOriginalWidth = `
ffmpeg -y -i ${inputPath} -preset veryslow -g 48 -crf 17 -sc_threshold -map 0:0 -map 0:1 -map 0:0 -map 0:1 -map 0:0 -map 0:1-s:v:0 ${getWidth(
    720,
    resolution
  )}x720 -c:v:0 libx264 -b:v:0 ${bitrate720}-s:v:1 ${getWidth(
    1080,
    resolution
  )}x1080 -c:v:1 libx264 -b:v:1 ${bitrate1080}-c:v:2 libx264 -b:v:2 ${bitrate}-c:a copy-var_stream_map "v:0,a:0 v:1,a:1 v:2,a:2"-master_pl_name master.m3u8-f hls -hls_time 6 -hls_list_size 0-hls_segment_filename "${outputSegmentPath}"${outputPath}
`
